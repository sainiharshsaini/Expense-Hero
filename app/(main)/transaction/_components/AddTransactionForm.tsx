...
export function AddTransactionForm({
	...
	return (
		<div className="animate-in fade-in slide-in-from-bottom-5 duration-700">
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-8 glass-panel border-white/10 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
				...
				<div className="grid gap-8 md:grid-cols-2">
					<div className="space-y-4">
						<label className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Transaction Type</label>
						<Select
						...
						>
							...
							<SelectContent className="glass-panel border-white/10">
								<SelectItem value="EXPENSE" className="text-red-400 font-bold focus:bg-red-400/10">Expense</SelectItem>
								<SelectItem value="INCOME" className="text-emerald-400 font-bold focus:bg-emerald-400/10">Income</SelectItem>
							</SelectContent>
						</Select>
						{errors.type && (
							<p className="text-sm text-red-500 animate-in shake duration-300">{errors.type.message}</p>
						)}
					</div>

					<div className="space-y-4">
						<label className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Amount</label>
						...
						{errors.amount && (
							<p className="text-sm text-red-500 animate-in shake duration-300">{errors.amount.message}</p>
						)}
					</div>
				</div>
				...
				<div className="grid gap-8 md:grid-cols-2">
					<div className="space-y-4">
						<label className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Account</label>
						<Select
							...
							<SelectContent className="glass-panel border-white/10">
								{accounts.map((account: any) => (
									<SelectItem key={account.id} value={account.id} className="font-medium">
										{account.name} <span className="text-muted-foreground ml-2">(${parseFloat(account.balance).toFixed(2)})</span>
									</SelectItem>
								))}
								<CreateAccountDrawer>
									<Button
						...
						{errors.accountId && (
							<p className="text-sm text-red-500 animate-in shake duration-300">{errors.accountId.message}</p>
						)}
					</div>
					...
					<div className="space-y-4">
						...
						{errors.category && (
							<p className="text-sm text-red-500 animate-in shake duration-300">{errors.category.message}</p>
						)}
					</div>
				</div>
				...
				<div className="grid gap-8 md:grid-cols-2">
					<div className="space-y-4">
						...
						{errors.date && (
							<p className="text-sm text-red-500 animate-in shake duration-300">{errors.date.message}</p>
						)}
					</div>
					...
					<div className="space-y-4">
						...
						/>
						{errors.description && (
							<p className="text-sm text-red-500 animate-in shake duration-300">{errors.description.message}</p>
						)}
					</div>
				</div>
				...
				<div className="flex flex-row items-center justify-between rounded-2xl border border-white/10 px-6 py-5 bg-white/5 hover:bg-white/10 transition-colors group">
					<div className="space-y-1">
						<label className="text-lg font-bold tracking-tight">Recurring Transaction</label>
				...
				{isRecurring && (
					<div className="space-y-4 animate-in slide-in-from-top-4 duration-500">
						<label className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Recurring Interval</label>
						<Select
						...
						>
							...
							<SelectContent className="glass-panel border-white/10">
								<SelectItem value="DAILY" className="font-medium">Daily</SelectItem>
