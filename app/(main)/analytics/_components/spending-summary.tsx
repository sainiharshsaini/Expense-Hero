		...
		<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
			<div className="stat-card group">
				<div className="flex items-center justify-between mb-4">
					...
					{incomeChange !== 0 && (
						<span
							className={`stat-trend ${incomeChange >= 0 ? "positive" : "negative"}`}
						>
							{incomeChange >= 0 ? (
				...
				<p className="text-xs text-muted-foreground mt-2">
					This month: $
					{currentMonth.income.toLocaleString(undefined, {
			...
			<div className="stat-card group">
				<div className="flex items-center justify-between mb-4">
					...
					{expenseChange !== 0 && (
						<span
							className={`stat-trend ${expenseChange <= 0 ? "positive" : "negative"}`}
						>
							{expenseChange <= 0 ? (
								<ArrowDownRight className="h-3 w-3" />
							) : (
				...
				<p className="text-xs text-muted-foreground mt-2">
					This month: $
					{currentMonth.expenses.toLocaleString(undefined, {
						maximumFractionDigits: 0,
					})}
				</p>
			</div>
			...
			<div className="stat-card group">
				...
				>
					...
					{Math.abs(totalIncome - totalExpenses).toLocaleString(undefined, {
						maximumFractionDigits: 0,
					})}
				</p>
				<p className="text-xs text-muted-foreground mt-2">
			...
			<div className="stat-card group">
				...
				<p className="stat-value text-indigo-500">
					$
					{avgMonthlySpending.toLocaleString(undefined, {
						maximumFractionDigits: 0,
					})}
				</p>
				<p className="text-xs text-muted-foreground mt-2">
