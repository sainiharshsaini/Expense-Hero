		...
		<Card className="glass-panel border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-700">
			<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-7 bg-white/5 border-b border-white/10">
				<div>
					...
					<p className="text-sm text-muted-foreground font-medium mt-1">Real-time spending analysis</p>
				</div>
			...
			<CardContent className="pt-8">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
					<div className="text-center p-6 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 hover:bg-emerald-500/10 transition-colors group">
						<p className="text-xs font-black uppercase tracking-widest text-emerald-500/60 mb-2">Total Income</p>
					...
					<div className="text-center p-6 rounded-2xl bg-red-500/5 border border-red-500/10 hover:bg-red-500/10 transition-colors group">
						<p className="text-xs font-black uppercase tracking-widest text-red-500/60 mb-2">Total Expenses</p>
					...
					<div className="text-center p-6 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 hover:bg-indigo-500/10 transition-colors group">
						<p className="text-xs font-black uppercase tracking-widest text-indigo-500/60 mb-2">Net Cash Flow</p>
						<p className={cn(
							"text-3xl font-black tabular-nums",
							totals.income - totals.expense >= 0 ? "text-primary" : "text-amber-400"
						)}>
							${(totals.income - totals.expense).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
						</p>
						<div className={cn(
							"mt-2 h-1 w-12 mx-auto rounded-full opacity-40 group-hover:w-20 transition-all duration-500",
							totals.income - totals.expense >= 0 ? "bg-primary" : "bg-amber-400"
						)} />
					</div>
				</div>
					...
					<ResponsiveContainer width="100%" height="100%">
						...
						>
									...
									if (active && payload && payload.length) {
										return (
											<div className="glass-panel border-white/10 p-4 shadow-2xl min-w-[150px]">
												<p className="text-xs font-black uppercase text-muted-foreground mb-2">{payload[0].payload.date}</p>
												{payload.map((p: any) => (
													<div key={p.dataKey} className="flex justify-between items-center gap-4 py-1">
														...
														<span className="text-sm font-black tabular-nums" style={{ color: p.fill }}>
							...
							<Legend
								...
								height={36}
								iconType="circle"
								formatter={(value) => <span className="text-xs font-black text-muted-foreground mr-4 translate-y-[-2px] inline-block">{value}</span>}
							/>
							<Bar
								dataKey="income"
								name="Income"
